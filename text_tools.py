...
def get_width_from_text(text, settings, mode="lyric"):
...
def draw_text_with_bbox(text, settings, mode="lyric", output_path=None):
    ...
    for t in text:
        _data = get_width_from_text(t, settings=settings, mode=mode)
        _bbox = _data["bbox"]
        _width = _data["width"]
...
def draw_lyric_image_with_ruby(data, settings, output_path_1="images/output_1.png", output_path_2="images/output_2.png"):
    ...
    for i, (lyric, ruby) in enumerate(zip(data["lyrics"], data["rubys"])):
        _width_lyric = draw_text_with_bbox(lyric, settings=settings, mode="lyric")
        _width_ruby = draw_text_with_bbox(ruby, settings=settings, mode="ruby")

        if lyric == " ":
            x_start_lyric.append(int(x_base))
            x_start_ruby.append(int(x_base))
            x_base += settings.LYRIC.MARGIN_SPACE
            x_end_lyric.append(int(x_base))
            x_end_ruby.append(int(x_base))
            continue

        elif lyric == "":
            x_start_lyric.append(int(x_base - settings.LYRIC.STROKE_WIDTH))
            x_start_ruby.append(int(x_base - settings.RUBY.STROKE_WIDTH))
            x_end_lyric.append(int(x_base - settings.LYRIC.STROKE_WIDTH))
            x_end_ruby.append(int(x_base - settings.RUBY.STROKE_WIDTH))
            continue
        ...
        # draw lyric
        x_start_lyric.append(int(_x_lyric - settings.LYRIC.STROKE_WIDTH))
        _margin = 0
        ...
        for t in lyric:
            ...
            _data = get_width_from_text(t, settings=settings, mode="lyric")
            _bbox = _data["bbox"]
            _width = _data["width"]
            _margin = _data["margin"]
        ...
        # draw ruby
        x_start_ruby.append(int(_x_ruby - settings.RUBY.STROKE_WIDTH))
        _margin = 0
        for t in ruby:
            _data = get_width_from_text(t, settings=settings, mode="ruby")
            _bbox = _data["bbox"]
            _width = _data["width"]
            _margin = _data["margin"]
